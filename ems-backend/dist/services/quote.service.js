...
exports.QuoteService = {
    ...
    async getDailyQuote() {
        ...
        let dailyQuote = await Quote_model_1.Quote.findOne({
        ...
        try {
            ...
            if (data && data.length > 0) {
                ...
                const newQuote = await Quote_model_1.Quote.create({
                    text: selected.q,
        ...
        try {
            return await Quote_model_1.Quote.create(fallbackDb);
        }
        catch (e) {
            return fallbackDb;
        }
    ...
    async getAllQuotes() {
        return Quote_model_1.Quote.find().sort({ createdAt: -1 });
    },
    async createQuote(data) {
        return Quote_model_1.Quote.create(data);
    },
    async deleteQuote(id) {
        const quote = await Quote_model_1.Quote.findById(id);
        if (!quote)
            throw new error_middleware_1.AppError('Quote not found', 404);
        await Quote_model_1.Quote.findByIdAndDelete(id);
    }
};
